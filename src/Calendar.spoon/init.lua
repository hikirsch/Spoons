...
local function updateCalCanvas()
    ...
    local needed_rownum = math.ceil((weekday_of_firstday+maxday_of_currentmonth)/7)

    for i=1,needed_rownum do
        for k=1,7 do
            local caltable_idx = 7*(i-1)+k
            local pushbacked_value = caltable_idx - weekday_of_firstday + 1
            ...
            if pushbacked_value == math.tointeger(current_day) then
                obj.canvas[52].frame.x = tostring((10+(obj.calw-20)/7*(k - 1))/obj.calw)
                obj.canvas[52].frame.y = tostring((10+(obj.calh-20)/7*(i+1))/obj.calh)
            end
        end
    end
    -- trim the canvas
...
function obj:init()
    ...
    obj.canvas[2] = {
        ...
        frame = {
            ...
            h = tostring((obj.calh-20)/7/obj.calh)
        }
    }
    ...
    for i=1,#weeknames do
        obj.canvas[2+i] = {
            ...
            textAlignment = "center",
            frame = {
                x = tostring((10+(obj.calw-20)/7*(i-1))/obj.calw),
                y = tostring((10+(obj.calh-20)/7)/obj.calh),
                w = tostring((obj.calw-20)/7/obj.calw),
                h = tostring((obj.calh-20)/7/obj.calh)
            }
        }
    end
    ...
    for i=1,6 do
        for k=1,7 do
            obj.canvas[9+7*(i-1)+k] = {
                ...
                textAlignment = "center",
                frame = {
                    x = tostring((10+(obj.calw-20)/7*(k-1))/obj.calw),
                    y = tostring((10+(obj.calh-20)/7*(i+1))/obj.calh),
                    w = tostring((obj.calw-20)/7/obj.calw),
                    h = tostring((obj.calh-20)/7/obj.calh)
                }
            }
    ...
    obj.canvas[52] = {
        ...
        frame = {
            x = tostring((10+(obj.calw-20)/7)/obj.calw),
            y = tostring((10+(obj.calh-20)/7*2)/obj.calh),
            w = tostring((obj.calw-20)/7/obj.calw),
            h = tostring((obj.calh-20)/7/obj.calh)
        }
    }
